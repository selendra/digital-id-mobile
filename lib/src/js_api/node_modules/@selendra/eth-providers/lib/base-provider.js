...
class BaseProvider extends abstract_provider_1.Provider {
    constructor({ safeMode = false, localMode = false, richMode = false, verbose = false, subqlUrl, maxBlockCacheSize = 200, storageCacheSize = 5000, healthCheckBlockDistance = 100 } = {}) {
        ...
        this.startSubscription = async () => {
            ...
            subscriptionMethod(async (header) => {
                ...
                if (this._listeners[NEW_LOGS]?.length > 0) {
                    const block = await this.getBlockData(blockNumber, false);
                    const receipts = await Promise.all(block.transactions.map((tx) => this.getTransactionReceiptAtBlock(tx, blockNumber)));
        ...
        this._getBlock = async (_blockTag) => {
            ...
            const normalTxHashes = evmExtrinsicIndexes.map((extrinsicIndex) => block.block.extrinsics[extrinsicIndex].hash.toHex());
            const allEvents = await this.queryStorage('system.events', [], blockHash);
            const virtualHashes = (0, utils_1.getVirtualTxReceiptsFromEvents)(allEvents, blockHash, blockNumber).map((r) => r.transactionHash);
            const alltxHashes = [...normalTxHashes, ...virtualHashes];
            return {
                hash: blockHash,
        ...
        this.getTransactionReceiptAtBlock = async (hashOrNumber, _blockTag) => {
            ...
            if (normalReceipt.status === 'fulfilled') {
                ...
                return utils_1.logger.throwError('receipt not found');
            }
        };
        this.getVirtualTxReceiptAtBlock = async (hashOrNumber, blockHash) => {
            if (typeof hashOrNumber !== 'string')
                return null;
            const blockNumber = (await this._getBlockHeader(blockHash)).number.toNumber();
            const allEvents = await this.queryStorage('system.events', [], blockHash);
            const virtualReceipt = (0, utils_1.getVirtualTxReceiptsFromEvents)(allEvents, blockHash, blockNumber).find((r) => r.transactionHash === hashOrNumber);
            if (!virtualReceipt)
                return null;
            return {
                ...virtualReceipt,
        ...
        this.getNormalTxReceiptAtBlock = async (hashOrNumber, blockHash) => {
            const blockNumber = (await this._getBlockHeader(blockHash)).number.toNumber();
        ...
        this.getTransactionByHash = async (txHash) => {
            ...
            try {
                ...
                extraData = (0, utils_1.parseExtrinsic)(extrinsic);
            }
            catch (e) {
                // virtual tx
                extraData = {
        ...
        this.getOrphanLogsAtBlock = async (blockTag) => {
            const blockHash = await this._getBlockHash(blockTag);
            const blockNumber = (await this._getBlockHeader(blockHash)).number.toNumber();
            const allEvents = await this.queryStorage('system.events', [], blockHash);
            const virtualReceipts = (0, utils_1.getVirtualTxReceiptsFromEvents)(allEvents, blockHash, blockNumber);
            const orphanLogs = virtualReceipts.reduce((logs, receipt) => logs.concat(receipt.logs), []);
            return (0, utils_1.hexlifyRpcResult)(orphanLogs);
        };
